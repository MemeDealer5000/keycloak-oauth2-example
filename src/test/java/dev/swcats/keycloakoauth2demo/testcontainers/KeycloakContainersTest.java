...
public class KeycloakContainersTest {
    ...
    public void testAuthenticateAdmin() throws Exception {
        ...
        Response response = RestAssured.given()
                ...
                .body(formData)
                .post("/realms/app-login/protocol/openid-connect/token");
        val token = response.body().as(Map.class);
        this.mockMvc
                .perform(MockMvcRequestBuilders.get("/api/admin").header("Authorization", "Bearer "+token.get("access_token")))
                .andDo(print())
    ...
    public void testAuthenticateUser() throws Exception {
        ...
        val token = response.body().as(Map.class);
        this.mockMvc
                .perform(MockMvcRequestBuilders.get("/api/user/sam").header("Authorization", "Bearer "+token.get("access_token")))
                .andDo(print())
                .andExpect(MockMvcResultMatchers.status().is2xxSuccessful());
    }

    @Test
    public void testAnonAuthenticate() throws Exception {
        String formData = "grant_type=password&client_id=app-login&username=petrovich&password=321321";
        Response response = RestAssured.given()
                .header("Content-Type", "application/x-www-form-urlencoded")
                .body(formData)
        ...
        val token = response.body().as(Map.class);
        this.mockMvc
                .perform(MockMvcRequestBuilders.get("/api/anonymous").header("Authorization", "Bearer "+token.get("access_token")))
                .andDo(print())
                .andExpect(MockMvcResultMatchers.status().is2xxSuccessful());
    }
}
